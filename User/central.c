...
uint16_t Central_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    {
        if(centralProcedureInProgress == FALSE && centralCCCDIndex < centralCCCDCount)
        {
            attWriteReq_t req;

            req.cmd = FALSE;
            req.sig = FALSE;
            req.handle = centralCCCDList[centralCCCDIndex];
            req.len = 2;
            req.pValue = GATT_bm_alloc(centralConnHandle, ATT_WRITE_REQ, req.len, NULL, 0);
            if(req.pValue != NULL)
            {
                ...
                PRINT("CCCD write %d/%d to handle 0x%04X\n",
                      centralCCCDIndex + 1, centralCCCDCount, req.handle);
    ...
    if(events & START_DISC_CHAR_EVT)
    {
        ...
        PRINT("ReadUsingCharUUID(0x2803) on 0x%x~0x%x\n", centralSvcStartHdl, centralSvcEndHdl);
        {
            bStatus_t ret = GATT_ReadUsingCharUUID(centralConnHandle, &req, centralTaskId);
            PRINT("ReadUsingCharUUID ret=%d\n", ret);
        }
...
static void centralProcessGATTMsg(gattMsgEvent_t *pMsg)
{
    ...
    {
        PRINT("DIAG: ReadByTypeRSP discState=%d numPairs=%d\n",
              centralDiscState, pMsg->msg.readByTypeRsp.numPairs);
    }
             ...
             (pMsg->msg.errorRsp.reqOpcode == ATT_WRITE_REQ)))
    {
        ...
        {
            uint8_t status = pMsg->msg.errorRsp.errCode;
            PRINT("Write Error: %02X on index %d\n", status, centralCCCDIndex);
            
            if (status == 0x03) {
                // Error 3: Write Not Permitted. ˵��ײ����ֻ���� Report Reference��
                // ����������ٶ� CCCD ����һ��������������������ԣ�
                PRINT("Hit Read-Only Descriptor! Guessing CCCD is at next handle...\n");
                centralCCCDList[centralCCCDIndex]++; // ����� 1
                centralProcedureInProgress = FALSE;
                tmos_start_task(centralTaskId, START_WRITE_CCCD_EVT, 16); // ���·���ǰ������д��
            } else {
                // ����δ֪����ֱ��������ǰͨ������֤������������������
                PRINT("Unknown error, skipping to next channel...\n");
                centralProcedureInProgress = FALSE;
                centralCCCDIndex++;
                if(centralCCCDIndex < centralCCCDCount) {
                    tmos_start_task(centralTaskId, START_WRITE_CCCD_EVT, 16);
                } else {
                    PRINT("All HID channels processed (with errors)!\n");
                }
            }
        }
        else
        {
            PRINT("Write CCCD %d success \n", centralCCCDIndex);
            centralProcedureInProgress = FALSE;
            centralCCCDIndex++;
            if(centralCCCDIndex < centralCCCDCount) {
                tmos_start_task(centralTaskId, START_WRITE_CCCD_EVT, 16);
    ...
    {
        uint16_t handle = pMsg->msg.handleValueNoti.handle;
        uint8_t len = pMsg->msg.handleValueNoti.len;
        uint8_t *data = pMsg->msg.handleValueNoti.pValue;

        PRINT("[Handle 0x%04X] Len: %d, Data: ", handle, len);
    ...
    {
        PRINT("Unhandled GATT: method 0x%04X discState %d\n", pMsg->method, centralDiscState);
    }
    GATT_bm_free(&pMsg->msg, pMsg->method);
}
...
static void centralEventCB(gapRoleEvent_t *pEvent)
{
    switch(pEvent->gap.opcode)
    {
        ...
        {
            uint8_t *pAdData = pEvent->deviceInfo.pEvtData;
            uint8_t adLen = pEvent->deviceInfo.dataLen;
            uint8_t idx = 0;

            while (idx < adLen) {
                uint8_t fieldLen = pAdData[idx];
                if (fieldLen == 0 || (idx + fieldLen) >= adLen) break;
                uint8_t fieldType = pAdData[idx + 1];
                if ((fieldType == GAP_ADTYPE_16BIT_MORE ||
                     fieldType == GAP_ADTYPE_16BIT_COMPLETE) &&
                    fieldLen >= 3) {
                    uint8_t uuidCount = (fieldLen - 1) / 2;
                    for (uint8_t i = 0; i < uuidCount; i++) {
                        uint16_t uuid = BUILD_UINT16(pAdData[idx + 2 + i * 2],
                                                     pAdData[idx + 3 + i * 2]);
                        if (uuid == 0x1812) {
...
static void centralStartDiscovery(void)
{
    uint8_t uuid[ATT_BT_UUID_SIZE] = {LO_UINT16(0x1812),
                                      HI_UINT16(0x1812)};
...
static void centralGATTDiscoveryEvent(gattMsgEvent_t *pMsg)
{
    ...
    else if(centralDiscState == BLE_DISC_STATE_CHAR)
    {
        ...
        {
            uint8_t numPairs = pMsg->msg.readByTypeRsp.numPairs;
            uint8_t *p = pMsg->msg.readByTypeRsp.pDataList;

            PRINT("Found %d characteristics:\n", numPairs);

            for(uint8_t i = 0; i < numPairs; i++)
            {
                uint16_t charHdl = BUILD_UINT16(p[0], p[1]);
                uint8_t  props   = p[2];
                uint16_t valHdl  = BUILD_UINT16(p[3], p[4]);
                uint16_t uuid    = BUILD_UINT16(p[5], p[6]);

                PRINT("  [%d] Hdl=0x%04X Props=0x%02X ValHdl=0x%04X UUID=0x%04X\n",
                      i, charHdl, props, valHdl, uuid);

                if(uuid == 0x2A4D && (props & 0x10))
                {
        ...
        {
            centralDiscState = BLE_DISC_STATE_IDLE;
            extern uint8_t centralProcedureInProgress;
            centralProcedureInProgress = FALSE;

            if(centralCCCDCount > 0)
            {
                centralCCCDIndex = 0;
                PRINT("Writing %d CCCD descriptors...\n", centralCCCDCount);
                tmos_start_task(centralTaskId, START_WRITE_CCCD_EVT, 16);
            }
            else
            {
                PRINT("ERROR: No Notify characteristic found!\n");
            }
        }
    }
    else
    {
        PRINT("GATT Disc: unhandled method 0x%04X state %d\n", pMsg->method, centralDiscState);
    }
}
/*********************************************************************
